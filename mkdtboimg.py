...
class Dtbo:
    ...
    def _read_dtbo_image(self):
        ...
        self.__metadata_size = (
            self.header_size + self.dt_entry_count * self.dt_entry_size
        )
        ...
        self.__file.seek(0)
        self.__metadata = struct.unpack(
            format_str, self.__file.read(self.__metadata_size)
        )
    ...
    def compress_dt_entry(self, compression_format, dt_entry_file):
        ...
        compress_gzip = zlib.compressobj(
            zlib.Z_DEFAULT_COMPRESSION, zlib.DEFLATED, self._GZIP_COMPRESSION_WBITS
        )  # gzip
    ...
    def add_dt_entries(self, dt_entries):
        ...
        dt_offset = self.header_size + dt_entry_count * self.dt_entry_size
    ...
    def extract_dt_file(self, idx, fout, decompress):
        ...
        if decompress and compression_format:
            if (
                compression_format == CompressionFormat.ZLIB_COMPRESSION
                or compression_format == CompressionFormat.GZIP_COMPRESSION
            ):
                fout.write(
                    zlib.decompress(
...
def parse_dt_entry(global_args, arglist):
    ...
    parser.add_argument(
        "dt_file", nargs="?", type=argparse.FileType("rb"), default=None
    )
    parser.add_argument(
        "--id", type=str, dest="id", action="store", default=global_args.id
    )
    parser.add_argument(
        "--rev", type=str, dest="rev", action="store", default=global_args.rev
    )
    parser.add_argument(
        "--flags", type=str, dest="flags", action="store", default=global_args.flags
    )
    parser.add_argument(
        "--custom0",
        type=str,
