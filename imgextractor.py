...
class Extractor:
    ...
    def __out_name(file_path, out=1):
        ...
        return (
            name.split("-")[0].split(" ")[0].split("+")[0].split("{")[0].split("(")[0]
        )
    ...
    def __ext4extractor(self):
        def scan_dir(root_inode, root_path=""):
            for entry_name, entry_inode_idx, entry_type in root_inode.open_dir():
                ...
                if entry_inode.is_dir:
                    dir_target = self.EXTRACT_DIR + entry_inode_path.replace(
                        " ", "_"
                    ).replace('"', "")
                    ...
                    file_target = self.EXTRACT_DIR + entry_inode_path.replace(
                        " ", "_"
                    ).replace('"', "")
                    try:
                    ...
                    try:
                        ...
                        if os.name == "posix":
                            ...
                            with open(target.replace("/", os.sep), "wb") as out:
                                out.write(
                                    b"!<symlink>"
                                    + link_target.encode("utf-16")
                                    + b"\x00\x00"
                                )
                                try:
                                    windll.kernel32.SetFileAttributesA(
                                        LPCSTR(target.encode()),
        ...
        self.__write(
            ...
            self.CONFIG_DIR + os.sep + self.FileName + "_size.txt",
        )
        with open(self.OUTPUT_IMAGE_FILE, "rb") as file:
            dir_r = self.FileName
            scan_dir(ext4.Volume(file).root)
            self.fs_config.insert(
                0, "/ 0 2000 0755" if dir_r == "vendor" else "/ 0 0 0755"
            )
            self.fs_config.insert(
                1,
                f"{dir_r} 0 2000 0755" if dir_r == "vendor" else "/lost+found 0 0 0700",
            )
            self.fs_config.insert(2 if dir_r == "system" else 1, f"{dir_r} 0 0 0755")
            self.__write(
                "\n".join(self.fs_config),
                self.CONFIG_DIR + os.sep + self.FileName + "_fs_config",
            )
            self.__write(
                "\n".join(self.space),
                os.path.join(self.CONFIG_DIR, self.FileName + "_space.txt"),
            )
            p1 = p2 = 0
            if self.context:
                self.context.sort()
                ...
                self.__write(
                    "\n".join(self.context),
                    self.CONFIG_DIR + os.sep + self.FileName + "_file_contexts",
                )
    ...
    def main(self, target: str, output_dir: str, work: str, target_type: str = "img"):
        ...
        with open(self.OUTPUT_IMAGE_FILE, "rb+") as file:
            ...
            if [True for i in [".", "@", "#"] if i in mount]:
                mount = ""
            if (
                self.__out_name(os.path.basename(output_dir)) != mount
                and mount
                and self.FileName != "mi_ext"
            ):
                print(
                    f"[N]:Filename appears to be wrong , We will Extract {self.OUTPUT_IMAGE_FILE} to {mount}"
                )
                self.EXTRACT_DIR = (
                    os.path.realpath(os.path.dirname(output_dir)) + os.sep + mount
                )
                self.FileName = mount
